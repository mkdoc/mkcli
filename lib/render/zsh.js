...
function cmdspec(program) {
  ...
  buf += '_arguments -C \\' + EOL;
  buf += indent + '"1:cmd:->cmds" \\' + EOL; 
  buf += indent + '"*::arg:->args" && ret=0' + EOL;

  buf += 'case $state in' + EOL;

  buf += indent + '(cmds)' + EOL;
  buf += indent + indent + '_values "commands" \\' + EOL;
  ...
  if(program.options) {
    buf += this.optspec(program, program.options);
  }
...
function optspec(program, options) {
  ...
  if(program.synopsis) {
    for(i = 0;i < program.synopsis.length;i++) {
      synopsis = program.synopsis[i];
  ...
  for(k in options) {
    ...
    if(opt.names.length > 1) {
      opts += '"(' + opt.names.join(' ') + ')"';
      opts += '{' + opt.names.join(isOption ? '=,' :  ',') + (isOption ? '=' : '') + '}';
