...
_notes(){
  ...
  #echo "normarg: $NORMARG"
  #echo "words: $words[@]"
  #echo "word: $words[$NORMARG-1]"
  #echo "line: $line"
  case "$words[$NORMARG-1]" in
    -*)
      ...
      _arguments \
        ...
        "--version[Print version and exit]" && ret=0;
    ;;
    *)
      _arguments \
        ":file:_files -g '*.md'" && ret=0;
    ;;
  esac
...
function esac(program, lead, opts) {
  var buf = ''
    , indent = this.indent
    , i
    , spec = []
  ...
  buf += lead + 'case "$words[$NORMARG-1]" in' + EOL;
  ...
  if(actions !== false) {
    ...
    buf += this.args(spec, lead + indent + indent);
  }
...
function cmdspec(program, lead, opts) {
  ...
  buf += this.esac(program, lead, opts);
  //buf += lead + indent + indent + ';;' + EOL;
