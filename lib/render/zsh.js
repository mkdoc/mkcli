...
function esac(program, lead, opts, depth) {
  depth = depth || 1;
  ...
  buf += lead + 'case "$words[' + depth + ']" in' + EOL;
  ...
  for(k in commands) {
    ...
    if(cmd.commands) {
      values = this.describe(cmd, cmd.commands, opts);

      buf += lead + indent + indent + 'commands=(' + EOL;
      buf += this.args(
        values, lead + indent + indent,
        {returns: false, keyword: false, esc: false});
      buf += EOL + lead + indent + indent + ')' + EOL;

      buf += lead + indent + indent
        + '_describe "' + cmd.name + ' commands" commands' + EOL;

      //values.unshift('"' + cmd.name + ' commands"');
      //buf += this.args(
        //values,
        //lead + indent + indent,
        ////{keyword: '_values', returns: this.required(program)});
        //{keyword: '_values'});

      // recurse for nested commands
      buf += this.esac(cmd, lead + indent + indent, opts, depth + 1); 
    }
...
function cmdspec(program, lead, opts) {
  ...
  if(program.commands) {
    ...
    buf += lead + indent + indent
      + '_describe "' + program.name + ' commands" commands' + EOL;
  ...
  if(program.commands) {
    buf += this.esac(program, lead + indent + indent, opts);
  }
...
function describe(program, map) {
  ...
  for(var k in map) {
    spec.push(
      '"' + map[k].name + ':' + map[k].description.replace(/:/g, '\\:') + '"'); 
  }
  return spec;
}
