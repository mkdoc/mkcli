...
function CliParser(opts) {
  ...
  this.value = typeof opts.value === 'function'
    ? opts.value : value;
}
...
function parse(list, id) {
  ...
  if(items.length) {
    // lazily create map of options or commands
  ...
  for(i = 0;i < items.length;i++) {
    ...
    if(opt.value) {
      var desc = typeof opt.description === 'string'
        ? opt.description : opt.description.literal;
      desc = this.value.call(this, opt, desc, opt.value);
      if(opt.description.literal) {
        opt.description.literal = desc;
      }else{
