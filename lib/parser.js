  ...
  if(state.nodes.length > 1) {
    ...
    switch(type) {
      case states.NAME:
        ...
        if(/\s/.test(state.data.name)) {
          state.data.parents = state.data.name.split(/\s/); 
          state.data.name = state.data.parents.pop();
        }

        state.data.names = [state.data.name];

        // skip the heading, add names from list declaration
