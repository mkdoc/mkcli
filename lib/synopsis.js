...
function synopsis(state, conf) {
  ...
  var literal = state.data.synopsis
    ...
    , opts = state.options ? state.options.data.options : null
    , cols = conf.cols || 80
    , indent = conf.indent || 0
    , wrap
    , info
    , str = ''
    , cre = /^(\[|<)(command)(\]|>) /im
  ...
  if(state.name.data.parents && state.name.data.parents.length) {
    name = state.name.data.parents.join(' ') + ' ' + name;
  }
  ...
  for(i = 0;i < literal.length;i++) {
    ...
    if(cols) {
      if(cre.test(src)) {
        // <command> | [command] + single space == 10
        indent += 10; 
      }
      wrap = wordwrap(indent + name.length + 1, cols)
    }
    ...
    str += name + ' ' + (wrap ? wrap(src).replace(/^\s+/, '') : src);
  }
